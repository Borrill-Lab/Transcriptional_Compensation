...
colnames(PTC_variants) <- c("CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT","C0604","C0895","C1015","C1704","CWT")
...
PTC_variants <- pivot_longer(PTC_variants, Variant1:Variant13, values_to="Variant")
...
PTC_variants$C0604 <- substr(PTC_variants$C0604, 0, 3)
PTC_variants$C0895 <- substr(PTC_variants$C0895, 0, 3)
PTC_variants$C1015 <- substr(PTC_variants$C1015, 0, 3)
PTC_variants$C1704 <- substr(PTC_variants$C1704, 0, 3)
PTC_variants$CWT <- substr(PTC_variants$CWT, 0, 3)

#Subset such that at least one of the lines has the non-PTC allele, and at least one has the PTC allele (not het)
PTC_variants_1 <- subset(PTC_variants, C0604=="0/0"|C0895=="0/0"|C1015=="0/0"|C1704=="0/0"|CWT=="0/0")
PTC_variants_1 <- subset(PTC_variants_1, C0604=="1/1"|C0895=="1/1"|C1015=="1/1"|C1704=="1/1"|CWT=="1/1")
#This leaves 374 variants

#Filter to keep only variants in canonical transcripts
PTC_variants_1_canonical <- subset(PTC_variants_1, grepl("YES",Variant))
#251 variants are kept
